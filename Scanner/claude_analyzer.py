...
class ClaudeAnalyzer:
    ...
    def _create_analysis_prompt(self, target_url, results, vulnerable_results, critical_vulns, high_vulns):
        ...
        prompt = f"""당신은 OWASP TOP 10과 KISA 보안 가이드를 전문으로 하는 웹 애플리케이션 보안 전문가입니다.
다음 취약점 스캔 결과를 분석하고 경영진과 개발팀 모두가 이해할 수 있는 전문적인 보안 분석 보고서를 작성해주세요.
...
다음 내용으로 전문적인 보안 분석 보고서를 작성해주세요:

### 1. 🎯 전체 요약 (Executive Summary)
- 전반적인 보안 상태 평가 (5단계: 매우 위험 / 위험 / 보통 / 양호 / 우수)
- 비즈니스 영향도 분석 (데이터 유출, 서비스 중단, 금전적 손실 가능성)
- 가장 시급한 조치가 필요한 취약점 TOP 3
